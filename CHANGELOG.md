...
* JSpecify: rewrite the diagnostic for a generic nullability mismatch, which now begins `incompatible nullability:` rather than `incompatible types:`. Where a `List<?>` passed to a `Collection<? extends Object>` parameter read as `incompatible types: List<?> cannot be converted to Collection<? extends Object> (List<?> is a subtype of Collection<?>)`, the message now says which type argument differs and why (#1822). It prints the source type and the required type with a caret under every position at fault, gives each position as a `path:` line (`Map type argument V -> wildcard upper bound`), says where a wildcard takes an implicit upper bound from, suggests a source type to write where explicit wildcard bounds repair the assignment, and at a variable declaration offers the required type carrying the source's nullness, by @vlsi (#1828)
...
* JSpecify: rewrite the diagnostic for a generic nullability mismatch, which now begins `incompatible nullability:` rather than `incompatible types:`. Where a `List<?>` passed to a `Collection<? extends Object>` parameter read as `incompatible types: List<?> cannot be converted to Collection<? extends Object> (List<?> is a subtype of Collection<?>)`, the message now says which type argument differs and why (#1822). It prints the source type and the required type with a caret under every position at fault, gives each position as a `path:` line (`Map type argument V -> wildcard upper bound`), says where a wildcard takes an implicit upper bound from, suggests a source type to write where explicit wildcard bounds repair the assignment, and at a variable declaration offers the required type carrying the source's nullness, by @vlsi (#1828)
* JSpecify: report a type argument whose declared upper bound admits null where a wildcard requires a non-null one, such as a `Box<T>` with `T extends @Nullable Object` passed to a `Box<? extends Object>` parameter. Such an assignment was accepted silently, by @vlsi (#1828)
