...
### Gradle
To integrate NullAway into your java project add the following to your `build.gradle` file:
...
Integrating with Android projects is mostly the same as the Java example above, except you don't need to use the `apt` plugin and can instead just declare it on the `annotationProcessor` configuration:
