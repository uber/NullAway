...
public class Vector<E extends @Nullable Object> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
    ...
    public synchronized void setSize(int newSize) {
        ...
        for (int to = elementCount, i = newSize; i < to; i++) es[i] = null;
    ...
    public synchronized int indexOf(@Nullable Object o, int index) {
        if (o == null) {
            for (int i = index; i < elementCount; i++) if (elementData[i] == null)
                return i;
        } else {
            for (int i = index; i < elementCount; i++) if (o.equals(elementData[i]))
                return i;
        }
        return -1;
    }
    ...
    private synchronized boolean bulkRemove(Predicate<? super E> filter) {
        ...
        if (i < end) {
            ...
            // set bit 0
            deathRow[0] = 1L;
            for (i = beg + 1; i < end; i++) if (filter.test(elementAt(es, i)))
                setBit(deathRow, i - beg);
            if (modCount != expectedModCount)
                throw new ConcurrentModificationException();
            modCount++;
            int w = beg;
            for (i = beg; i < end; i++) if (isClear(deathRow, i - beg))
                es[w++] = es[i];
    ...
    private void shiftTailOverGap(Object[] es, int lo, int hi) {
        ...
        for (int to = elementCount, i = (elementCount -= hi - lo); i < to; i++) es[i] = null;
    }
    ...
    private class Itr implements Iterator<E> {
        ...
        public void forEachRemaining(Consumer<? super E> action) {
            ...
            synchronized (Vector.this) {
                ...
                if (i >= es.length)
                    throw new ConcurrentModificationException();
                while (i < size && modCount == expectedModCount) action.accept(elementAt(es, i++));
    ...
    public synchronized void replaceAll(UnaryOperator<E> operator) {
        ...
        final Object[] es = elementData;
        final int size = elementCount;
        for (int i = 0; modCount == expectedModCount && i < size; i++) es[i] = operator.apply(elementAt(es, i));
        if (modCount != expectedModCount)
            throw new ConcurrentModificationException();
        modCount++;
    }
    ...
    final class VectorSpliterator implements Spliterator<E> {
        ...
        public void forEachRemaining(Consumer<? super E> action) {
            ...
            int i;
            for (i = index, index = hi; i < hi; i++) action.accept((E) a[i]);
            if (modCount != expectedModCount)
                throw new ConcurrentModificationException();
        }
