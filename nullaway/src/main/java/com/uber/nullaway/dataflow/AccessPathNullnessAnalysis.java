...
public final class AccessPathNullnessAnalysis {
    ...
    private AccessPathNullnessAnalysis(
        ...
        this.nullnessPropagation = new AccessPathNullnessPropagation(
     ...
     * @param methodReturnsNonNull predicate determining whether a method is assumed to return
     * NonNull value
     * @param types javac Types data structure
     * @param config analysis config
     * @return instance of the analysis
     */
    public static AccessPathNullnessAnalysis instance(Context context,
            Predicate<MethodInvocationNode> methodReturnsNonNull,
            Types types,
            Config config,
            Handler handler
    ...
    public Set<Element> getNonnullFieldsOfReceiverAtExit(TreePath methodPath, Context context) {
        NullnessStore<Nullness> nullnessResult = dataFlow.finalResultForMethod(methodPath, context,
                nullnessPropagation);
