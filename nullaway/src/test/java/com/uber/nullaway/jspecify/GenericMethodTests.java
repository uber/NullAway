...
public class GenericMethodTests extends NullAwayTestsBase {
  ...
  /**
   * Source of the file that each attribution test compiles first, padded so that a diagnostic
   * misattributed to it lands on an existing line. The test framework drops a diagnostic that
   * points past the end of a file, so without the padding a test passes with the misattribution in
   * place.
   */
  ...
  /**
   * Same as {@link #inferenceFailureReportedInFileWithCall()}, for a failure that only dataflow
